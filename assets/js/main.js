...
const Memorial = {
  ...
  music() {
    ...
    // Attempt autoplay immediately; if blocked by browser policy,
...
function initFlowerForm() {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      const res = await memorialPost(
        SITE_URL + "/api/deposit_flower.php",
        data,
      );
      if (res.success) {
        showAlert("🌹 Your flowers have been placed with love.", "success");
        bootstrap.Modal.getInstance(
          document.getElementById("flowerModal"),
        )?.hide();
        // Add flower visually
